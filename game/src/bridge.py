...
class Bridge:
                ...
                while '\n' in buffer:
                    ...
                    if message.strip():
                        try:
                            ...
                            self._send_error(client_socket, "Invalid JSON format", str(e))
    ...
    def _process_message(
        ...
        try:
            if msg_type == MessageType.CONNECT:
                ...
                if not game:
                    self._send_error(client_socket, "No active game", "Send CONNECT first")
                    return None
                ...
                self._send_error(client_socket, "Unknown message type", f"Type: {msg_type}")
                return None
    ...
    def _handle_connect(self, client_id: str, client_socket: socket.socket, json_data: Dict) -> Dict:
        ...
        game = Game(client_id, opponent_type=opponent_type, render_mode=self.render_mode)
    ...
    def _handle_action(self, client_socket: socket.socket, json_data: Dict, game: Game) -> None:
        ...
        if action is None:
            self._send_error(client_socket, "Missing action", "Action field is required")
            return
