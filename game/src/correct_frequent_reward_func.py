...
class WinLoseRewardFn(RewardFn):
    ...
    def __call__(self, prior_obs: Observation, prior_action: Action, obs: Observation) -> float:
        change_in_num_generals_owned = compute_num_generals_owned(
            obs) - compute_num_generals_owned(prior_obs)
...
class FrequentAssetRewardFn(RewardFn):
    ...
    def __call__(self, prior_obs: Observation, prior_action: Action, obs: Observation) -> float:
        ...
        change_in_num_cities_owned = compute_num_cities_owned(
            obs) - compute_num_cities_owned(prior_obs)
        change_in_num_generals_owned = compute_num_generals_owned(
            obs) - compute_num_generals_owned(prior_obs)
        # Moderately reward valid actions & penalize invalid actions.
        valid_action_reward = 1 if is_action_valid(
            prior_action, prior_obs) else -5
